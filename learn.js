(() => {
  ...
  function renderSegmentTeachingMedia(lessonIndex,seg){
    ...
    const lead=placements[0],rest=placements.slice(1),requiredRest=rest.filter(p=>p.requirement==='required'),supportingRest=rest.filter(p=>p.requirement!=='required');
    const requiredGroup=requiredRest.length?`<section class="lesson-required-resources" aria-label="Required resources for this section"><div class="lesson-resource-group-heading"><strong>Required resources for this section</strong><span>${requiredRest.length} additional required source${requiredRest.length===1?'':'s'}</span></div><div class="lesson-video-more-grid">${requiredRest.map(p=>lessonResourceCard(p,{compact:true})).join('')}</div></section>`:'';
    const supportingGroup=supportingRest.length?`<details class="lesson-video-more lesson-supporting-resources"><summary>Optional / supporting resources (${supportingRest.length})</summary><div class="lesson-video-more-grid">${supportingRest.map(p=>lessonResourceCard(p,{compact:true})).join('')}</div></details>`:'';
    return `<section class="lesson-teaching-media" aria-label="Teaching resources for ${esc(seg.label)}"><div class="lesson-teaching-media-head"><span>Teaching resources · placed at point of use</span><h3>Use the supporting source where it helps</h3><p>Alfred teaches the model here. Required sources are always visible; optional and supporting resources stay compact until you choose them.</p></div>${lessonResourceCard(lead)}${requiredGroup}${supportingGroup}</section>`;
  }
