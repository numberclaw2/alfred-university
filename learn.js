(() => {
  ...
  const MC = window.AlfredMediaConsumption || {};
  ...
  function literatureIntegration(){ return C.outsideLiteratureIntegration || {sources:{},byPlacement:{},bySource:{},sourceIds:[]}; }
  function resourcePlacementKey(p){ return `${p.assignmentWeek}:${p.source}:${p.targetWeek}:${p.lesson}:${p.segment}`; }
  function literaturePlacementMeta(p){ return literatureIntegration().byPlacement?.[resourcePlacementKey(p)] || null; }
  ...
  function teachingMediaFilterType(sourceId){
    const mediaType=teachingMediaType(sourceId),s=source(sourceId),kind=String(s.kind||'').toLowerCase();
    if(mediaType==='video')return 'video';
    if(mediaType==='literature'||isLiteratureSource(sourceId))return 'reading';
    if(/interactive|simulation|simulator|calculator|virtual lab|web tool/.test(kind))return 'interactive';
    return 'reference';
  }
  function teachingMediaFilterTypeLabel(type){return {video:'Video',reading:'Written literature',interactive:'Interactive / tool',reference:'Reference / guide'}[type]||'Resource';}
  function teachingMediaCardPlacements(sourceId){
    const all=globalResourcePlacements(sourceId),current=all.filter(p=>Number(p.assignmentWeek)===Number(week));
    return current.length?current:all;
  }
  function teachingMediaTrack(sourceId){
    const tracks=new Set(teachingMediaCardPlacements(sourceId).map(p=>Number(p.lesson)===0?'ceta':'career'));
    return tracks.size>1?'both':tracks.has('career')?'career':'ceta';
  }
  function teachingMediaRoleTokens(sourceId){return [...new Set(teachingMediaCardPlacements(sourceId).map(p=>p.presentationRole).filter(Boolean))];}
  function teachingMediaBeginnerFriendly(sourceId,item){
    if(BEGINNER_MEDIA_IDS.has(sourceId))return true;
    const s=source(sourceId),litSource=literatureIntegration().sources?.[sourceId]||null,litRows=literatureIntegration().bySource?.[sourceId]||[];
    const text=[s.title,s.kind,item?.role,litSource?.provenance,...litRows.map(x=>x.why)].filter(Boolean).join(' ').toLowerCase();
    return /\bbeginner[- ]friendly\b|\bbeginner[- ]first\b|\bfor beginners\b|\bbeginner (?:youtube|tutorial|lecture|guide|article|reading)\b|\bstart here\b|\bclarity\b/.test(text);
  }
  ...
  function lessonResourceCard(placement,{compact=false}={}){
    ...
    if(!item)return '';
    const lit=literaturePlacementMeta(placement),isLit=!!lit;
    const litSourceMeta=literatureIntegration().sources?.[placement.source]||null;
    const youtubeId=!isLit&&placement.mediaType==='video'?youtubeVideoId(s.url):'';
    ...
    const sourceButton=s.url?`<a class="button outline-green" href="${esc(s.url)}" target="_blank" rel="noopener noreferrer">${isLit?'Open reading':placement.mediaType==='video'?'Open source':'Open reference'} ↗</a>`:'<span class="private-source">Use your authorized private copy; it is not republished here.</span>';
    const roleLabel=isLit?(lit.literatureType||litSourceMeta?.literatureType||literatureIntegration().existingType?.[placement.source]||'Written companion'):resourceRoleLabel(placement.presentationRole);
    const authorLine=isLit?`<p class="literature-source-line">${litSourceMeta?.author?`<span>${esc(litSourceMeta.author)}</span> · `:''}<span>${esc(s.org)}</span>${litSourceMeta?.access?` · <span>${esc(litSourceMeta.access)}</span>`:''}</p>`:`<p class="lesson-video-org">${esc(s.org)}</p>`;
    const readUse=isLit?(litSourceMeta?lit.readUse:item.watchFor):item.watchFor;
    const focus=isLit?(lit.focus||item.use):'';
    const after=isLit?(lit.afterReading||placement.afterAction):placement.afterAction;
    const framing=isLit?`<div class="lesson-video-framing literature-framing"><p><strong>Why this reading is here:</strong> ${esc(lit.why||item.use)}</p><p><strong>Read / use:</strong> ${esc(readUse)}</p><p><strong>Focus on:</strong> ${esc(focus)}</p><p><strong>After reading:</strong> ${esc(after)}</p></div>`:`<div class="lesson-video-framing"><p><strong>Why this is here:</strong> ${esc(item.use)}</p><p><strong>Watch for:</strong> ${esc(item.watchFor)}</p><p><strong>Next:</strong> ${esc(placement.afterAction)}</p></div>`;
    const consumptionKind=isLit?'reading':placement.mediaType==='video'?'video':teachingMediaFilterType(placement.source)==='interactive'?'interactive':'reference';
    const consumption=MC.controlHTML?.({week:placement.assignmentWeek,id:MC.mediaId?.(placement.assignmentWeek,placement.source),kind:consumptionKind,compact})||'';
    return `<article class="lesson-video-card lesson-resource-card${isLit?' literature-card':''}${compact?' compact':''}" id="${esc(cardId)}" data-resource-source="${esc(placement.source)}"><div class="lesson-video-card-top"><span class="lesson-video-role role-${esc(placement.presentationRole)}">${esc(roleLabel)}</span><span class="lesson-video-requirement requirement-${esc(placement.requirement)}">${esc(resourceRequirementLabel(placement))}</span></div><h4>${esc(s.title)}</h4>${authorLine}${framing}<div class="lesson-video-actions">${inline}${sourceButton}<a class="lesson-video-library-link" href="learn.html?week=${placement.assignmentWeek}&stage=media#media-${encodeURIComponent(teachingMediaAnchorSource(placement.source,placement.assignmentWeek))}">Teaching Media entry</a></div>${consumption}${youtubeId?`<div class="lesson-video-inline hidden" id="${esc(cardId)}-frame" data-inline-frame="${esc(cardId)}"><div class="lesson-video-aspect"><iframe title="${esc(s.title)}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div></div>`:''}</article>`;
  }
  function studyGuideLabel(record){return SG.pageLabel?SG.pageLabel(record):(record.pageGroups||[]).map(g=>g[0]===g[1]?`p.${g[0]}`:`pp.${g[0]}–${g[1]}`).join(', ');}
  function studyGuidePrimaryHref(record){const p=SG.primaryPlacement?.(record);return p?`learn.html?week=${p.week}&stage=${p.lesson===0?'ceta-lesson':'career-lesson'}&lesson=${p.lesson}&section=${encodeURIComponent(p.segment)}`:`learn.html?week=${record.week}&stage=media`;}
  function studyGuideErrata(record){const rows=SG.errataFor?.(record)||[];return rows.map(x=>`<aside class="study-guide-errata" role="note"><strong>⚠ ${esc(x.label)}</strong><p>${esc(x.correction)}</p><a href="${esc(x.sourceUrl)}" target="_blank" rel="noopener">Open official ETA errata ↗</a></aside>`).join('');}
  function studyGuideCard(record,{compact=false,priorPass=false,context='lesson',backlink=false}={}){
    if(!record)return '';
    const required=record.classification==='required',historical=record.classification==='reference'||/historical/i.test(record.authorityNote||'');
    const classification=priorPass&&required?'review-added':backlink&&required?'study':record.classification;
    const badge=priorPass&&required?'Added since you completed this lesson — review recommended':backlink&&required?'Previously assigned · reuse if needed':required?'Required':record.classification==='study'?'Review if needed':'Reference / historical';
  ...
  function renderRelatedLearning(lessonIndex,seg,priorPass=false){
    const guide=segmentStudyGuideRecords(lessonIndex,seg),placements=segmentResourcePlacements(lessonIndex,seg.id).map(p=>({...p,segmentLabel:seg.label}));
    if(!guide.length&&!placements.length)return '';
    ...
    const requiredGuide=guide.filter(r=>r.classification==='required'),supportGuide=guide.filter(r=>r.classification!=='required');
    const requiredMedia=placements.filter(p=>p.requirement==='required'),supportMedia=placements.filter(p=>p.requirement!=='required');
    const isPrimary=r=>{const p=SG.primaryPlacement?.(r);return !!p&&Number(p.week)===Number(week)&&Number(p.lesson)===Number(lessonIndex)&&String(p.segment)===String(seg.id);};
    const guidePrimary=requiredGuide.filter(isPrimary),guideBacklinks=requiredGuide.filter(r=>!isPrimary(r));
    const required=[...guidePrimary.map(r=>studyGuideCard(r,{compact:true,priorPass})),...requiredMedia.map(p=>lessonResourceCard(p,{compact:true}))].join('');
    const support=[...guideBacklinks.map(r=>studyGuideCard(r,{compact:true,priorPass,backlink:true})),...supportGuide.map(r=>studyGuideCard(r,{compact:true,priorPass})),...supportMedia.map(p=>lessonResourceCard(p,{compact:true}))].join('');
    return `<section class="related-learning" aria-label="Related learning for ${esc(seg.label)}"><div class="related-learning-head"><span>Related Learning</span><h3>Use the extra representation only where it adds something</h3><p>Alfred remains the primary teacher. Required reinforcement stays visible; optional review stays collapsed.</p></div>${required?`<div class="related-learning-required">${required}</div>`:''}${support?`<details class="related-learning-support"><summary>Need another explanation or reference? (${supportGuide.length+supportMedia.length})</summary><div>${support}</div></details>`:''}</section>`;
  }
  function renderSegmentTeachingMedia(lessonIndex,seg,priorPass=false){return renderRelatedLearning(lessonIndex,seg,priorPass);}
  ...
  function resourceLessonHref(placement,sourceId){
    return `learn.html?week=${placement.targetWeek}&stage=${placement.lesson===0?'ceta-lesson':'career-lesson'}&lesson=${placement.lesson}&section=${encodeURIComponent(placement.segment)}&from=media&mediaWeek=${week}&media=${encodeURIComponent(sourceId)}`;
  }
  ...
  function reviewTargetsForSemanticTask(index,item){
    ...
    if(!targets.length){
      const teachingIndex=(moduleData.lessons[index]?.integrated?.teaching||[]).findIndex(section=>section.title===item.title);
      const mapped=teachingIndex>=0?plan.findIndex(seg=>seg.type==='teaching'&&seg.teachingIndex===teachingIndex):-1;
      const target=Math.max(1,Math.min(plan.length-1,mapped>=0?mapped:1));
  ...
  function lessonSegmentPlan(lessonItem,index){
    ...
    (d.teaching||[]).forEach((section,i)=>{if(section.hiddenFromLesson)return;segments.push({id:section.sectionId||`concept-${i+1}`,type:'teaching',label:section.title||`Concept ${i+1}`,section,teachingIndex:i,minutes:segmentMinutesForText(`${section.text||''} ${section.remember||''}`)});});
  ...
  function normalizeLegacyLessonSegmentRaw(index,plan,raw){
    if(Number(week)!==4||Number(index)!==0||Number(raw?.total)!==Number(plan.length)+1)return raw||{};
    const movedOldIndex=5,completed={};
    Object.entries(raw.completed||{}).forEach(([k,v])=>{if(!v)return;const n=Number(k);if(!Number.isFinite(n)||n===movedOldIndex)return;completed[String(n>movedOldIndex?n-1:n)]=true;});
    const shift=n=>{n=Number(n)||0;return n>movedOldIndex?n-1:n;};
    return {...raw,current:shift(raw.current),furthest:shift(raw.furthest??raw.current),total:plan.length,completed,sequenceMigration:'v16.3.53-w4-digital-to-w11'};
  }
  function lessonSegmentRecord(index,plan){
    const l=learningState().l,key=`lesson-${index}`,raw=normalizeLegacyLessonSegmentRaw(index,plan,l.lessonSegments?.[key]||{}),total=plan.length,stageId=index===0?'ceta-lesson':'career-lesson';
  ...
  function renderMedia(){
    ...
    const rows=items.map(item=>{const s=source(item.source),assignment=mediaArchitectureAssignment(item.source,week),requirement=assignment?.requirement||teachingMediaRequirement(item.source),mediaType=teachingMediaType(item.source),lit=literatureMediaMeta(item.source),isLit=!!lit,litSource=literatureIntegration().sources?.[item.source]||null;const readUse=isLit?(litSource?lit.readUse:item.watchFor):item.watchFor,focus=isLit?(lit.focus||item.use):'',after=isLit?(lit.afterReading||teachingMediaPrimaryPlacement(item.source)?.afterAction):'',typeLabel=isLit?(lit.literatureType||litSource?.literatureType||literatureIntegration().existingType?.[item.source]||s.kind):s.kind,filterType=teachingMediaFilterType(item.source),track=teachingMediaTrack(item.source),meta=assignment?.requiredMeta||item.architectureRequiredMeta||null;return {item,s,assignment,requirement,mediaType,lit,isLit,litSource,readUse,focus,after,typeLabel,filterType,track,meta};});
    const guideRequired=SG.requiredForWeek?.(week)||[],guideAll=SG.recordsForWeek?.(week)||[];
    const requiredCount=rows.filter(x=>x.requirement==='required').length,conditionalCount=rows.filter(x=>x.requirement==='conditional').length,totalRequired=requiredCount+guideRequired.length;
    ...
    const cards=rows.map(({item,s,requirement,mediaType,lit,isLit,litSource,readUse,focus,after,typeLabel,filterType,track,meta})=>{
      const consumptionKind=isLit?'reading':mediaType==='video'?'video':filterType==='interactive'?'interactive':'reference';
      const consumption=MC.controlHTML?.({week,id:MC.mediaId?.(week,item.source),kind:consumptionKind})||'';
    ...
    if(!rows.length&&!guideRequired.length)return `<div class="classroom-stage-head"><div><span class="stage-count">Stage 4 of ${STAGES.length} · Required path</span><h2>No outside Teaching Media or Study Guide reading is required this week</h2><p>Alfred's lesson, practice, and application already provide the first-pass teaching. Use Study only if you want another explanation, a refresher, troubleshooting help, or deeper material.</p></div>${trackBadge('CETa + Career')}</div><section class="media-policy"><strong>Your stopping point is explicit.</strong><p>You do not need to watch or read an outside resource before continuing. Move directly into practice and application.</p></section>${resourceMap}<section class="media-reflection"><button class="button green" id="complete-media" type="button">${stageComplete('media')?'✓ No outside media required — stage complete':'Continue — no outside media required'}</button></section>`;
  ...
  function bindStage(){
    ...
    MC.bind?.($('#classroom-content')||document);
    MC.refresh?.($('#classroom-content')||document);
  ...
  function loadWeek(nextWeek){
    week = nextWeek; moduleData = MODULES.find(m => m.week === week); lessonViewOverrides.clear(); lessonReferencePreviews.clear(); const reviewReturn=readReviewReturn(); if(reviewReturn&&Number(reviewReturn.week)!==Number(week))clearReviewReturn();
