(() => {
  ...
  function reviewTargetForQuestion(index,kind,questionIndex){const lesson=moduleData.lessons[index],plan=lessonSegmentPlan(lesson,index);const preferred=kind==='check'?lesson?.integrated?.checks?.[questionIndex]?.reviewSectionTitle:lesson?.knowledgeCheck?.reviewSectionTitle;let target=null;if(preferred){const teachingIndex=(lesson?.integrated?.teaching||[]).findIndex(section=>section.title===preferred);if(teachingIndex>=0)target=teachingIndex+1;}if(!Number.isFinite(target)){const raw=Number(QUESTION_REVIEW_MAP[`${week}:${index}:${kind}:${questionIndex}`]);target=Number.isFinite(raw)?Math.max(1,Math.min(plan.length-1,raw)):Math.max(1,Math.min(plan.length-1,plan.findIndex(x=>x.type==='teaching')));}target=Math.max(1,Math.min(plan.length-1,target));return {target,segment:plan[target]};}
  ...
  function reviewTargetForSemanticTask(index,item){const plan=lessonSegmentPlan(moduleData.lessons[index],index),teachingIndex=(moduleData.lessons[index]?.integrated?.teaching||[]).findIndex(section=>section.title===item.title),target=Math.max(1,Math.min(plan.length-1,teachingIndex>=0?teachingIndex+1:1));return {target,segment:plan[target]};}
  function renderSemanticReviewAction(index,item,anchor){const x=reviewTargetForSemanticTask(index,item);return `<div class="question-review-tools"><button class="button outline-green question-review-link" type="button" data-review-lesson="${index}" data-review-section="${x.target}" data-review-anchor="${esc(anchor)}">Review Section ${x.target+1} · ${esc(x.segment?.label||item.title||'Teaching section')}</button><small>This competency prompt uses material taught in that section. Review it first if the terminology or safety context is unfamiliar.</small></div>`;}
  ...
  function renderIntegratedLesson(lessonItem,index){
    ...
    return `<section class="integrated-lesson segmented-lesson" data-v="16.3.32">${renderLessonSegment(lessonItem,index)}</section>`;
  }
