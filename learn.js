(() => {
  ...
  function renderMedia(){
    ...
    <div class="teaching-media-list">${items.map((item,i) => {const s=source(item.source),requirement=teachingMediaRequirement(item.source),mediaType=teachingMediaType(item.source),cue=mediaType==='video'?'Watch for':'Read / use for';return `<article id="media-${esc(item.source)}"><div class="media-card-top"><span class="media-requirement-status requirement-${esc(requirement)}">${esc(resourceRequirementLabel({requirement}))}</span><span>${esc(item.role)}</span><span>${esc(s.kind)}</span></div><h3>${esc(s.title)}</h3><p class="media-org">${esc(s.org)}</p><p><strong>Use it for:</strong> ${esc(item.use)}</p><p><strong>${cue}:</strong> ${esc(item.watchFor)}</p><p><strong>What Alfred still supplies:</strong> ${esc(item.gap)}</p>${teachingMediaLessonLinks(item.source)}<div>${s.url ? `<a class="button outline-green" href="${esc(s.url)}" target="_blank" rel="noopener noreferrer">Open source ↗</a>` : '<span class="private-source">Use your private course copy; it is not republished here.</span>'}</div></article>`}).join('')}</div>
