...
(() => {
  ...
  D.majorAssessments.forEach(m => {
    if (m.id === 'ceta-mock') Object.assign(m,{requiredRuns:1,qualifyingTarget:85,independentRunRequired:true,independentRunTarget:85,readinessRule:'Complete one current full-length 100-question Alfred CETa practice run at 85% or higher plus one separate current independent CETa practice assessment at 85% or higher from outside the Alfred question bank. Repeating the Alfred mock does not count as the independent second gate. This is an internal readiness rule, not ETA’s official passing standard or a guarantee.'});
  });
