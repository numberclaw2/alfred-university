...
function renderStudyFigure(figure){
  ...
  if(figure.type==='flow'){const steps=figure.steps||[];return `<figure class="study-review-figure"><figcaption><strong>${esc(figure.number||'Career mental model')} · ${esc(figure.title||'')}</strong></figcaption><div class="source-figure-flow study-figure-flow" role="group" aria-label="${esc(figure.title||'Instructional flow')}">${steps.map((step,i)=>`<div class="source-flow-step"><span>${i+1}</span><div><strong>${esc(step.title||`Step ${i+1}`)}</strong><p>${esc(step.body||'')}</p></div></div>${i<steps.length-1?'<span class="source-flow-arrow" aria-hidden="true">→</span>':''}`).join('')}</div>${figure.caption?`<p>${esc(figure.caption)}</p>`:''}</figure>`;}
