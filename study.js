...
function targetEvent(){
  const params=new URLSearchParams(location.search); const q=params.get('event'); if(q){const hit=E.find(e=>String(e.id)===q);if(hit)return hit;}
  const requestedWeek=Number(params.get('week')||0); const t=now(); const incomplete=e=>eventStatus(e.id)!=='complete';
  if(requestedWeek&&W.some(w=>w.week===requestedWeek)){const wl=E.filter(e=>e.week===requestedWeek&&incomplete(e)).sort((a,b)=>new Date(a.start)-new Date(b.start)); if(wl.length)return wl.find(e=>new Date(e.start)>=t)||wl[0];}
  const todays=E.filter(e=>sameDay(new Date(e.start),t)&&incomplete(e)).sort((a,b)=>new Date(a.start)-new Date(b.start)); if(todays.length)return todays[0];
  const upcoming=E.filter(e=>new Date(e.start)>=t&&incomplete(e)).sort((a,b)=>new Date(a.start)-new Date(b.start)); if(upcoming.length)return upcoming[0];
  const w=currentWeek(); const catchup=E.filter(e=>e.week===w&&incomplete(e)).sort((a,b)=>new Date(b.start)-new Date(a.start)); if(catchup.length)return catchup[0];
  return E.find(incomplete)||E[E.length-1];
}
...
function renderRecommendation(){
  const box=$('#recommended-session'); if(!box||!target)return;
...
function renderWeeklyPlan(){
  const w=currentWeek(), wd=weekData(w), list=E.filter(e=>e.week===w), mins=list.filter(e=>e.type!=='Equipment').reduce((a,e)=>a+eventMinutes(e),0), lab=list.find(e=>e.type==='Lab'||e.type==='Project'); const p=progress(); const mastery=p.weeks?.[w]||'';
...
function buildSteps(e,mode){
  ...
  const steps=[];
  steps.push({title:`Recall · ${plan[0].minutes} min`,body:`<p class="step-directive"><strong>Before looking anything up:</strong> explain this from memory:</p><blockquote>${esc(recallPrompt)}</blockquote><p>If you cannot explain it yet, that is useful information. Do not grade yourself while recalling—just expose the gap.</p><button class="button outline-green" type="button" data-stuck>I'm Stuck</button>`});
  if(mode==='quick'){
    steps.push({title:`Core Move · ${plan[1].minutes} min`,body:`<p class="step-directive">Do only the highest-value core action for this session.</p><div class="session-resource-list"><a class="session-resource" href="week.html?week=${e.week}"><strong>Open Week ${String(e.week).padStart(2,'0')} Module</strong><span>Required outcomes and scheduled sequence</span></a>${resources[0]?`<a class="session-resource" href="${esc(resources[0].url)}" target="_blank" rel="noopener"><strong>${esc(resources[0].title)}</strong><span>${esc(resources[0].priority||'Core resource')}</span></a>`:''}</div><p class="quick-rule"><strong>Quick-session rule:</strong> stop when the timer is up. Momentum is the goal; the full task can continue in Standard or Deep Work.</p>`});
  }else{
    steps.push({title:`Learn · ${plan[1].minutes} min`,body:`<p class="step-directive">Use the required source first. Open alternatives only if the first explanation is not working.</p><div class="session-resource-list">${resourceHtml}</div><div class="resource-layer-note"><strong>Order:</strong> CETa Core → Career Core → Help → Deep Dive. Do not browse the full library unless the required explanation is insufficient.</div><button class="button outline-green" type="button" data-stuck>I'm Stuck</button>`});
    steps.push({title:`${plan[2].name} · ${plan[2].minutes} min`,body:`<p class="step-directive">Produce evidence. Watching or reading alone does not finish this step.</p><ul class="definition-list">${outcomes.slice(0,4).map(o=>`<li>${esc(o)}</li>`).join('')}</ul><div class="proof-box"><strong>Proof can be:</strong> a correct calculation, a measured value, a working circuit/code result, a truth table, a scope/logic capture, or a troubleshooting explanation.</div>${['Lab','Project'].includes(e.type)?'<a class="button green" href="labs.html">Open Lab Center</a>':''}`});
  }
...
function reviewDue(){const t=startOfDay(now()).getTime();return Object.values(state.reviews||{}).filter(r=>new Date(r.due).getTime()<=t).sort((a,b)=>new Date(a.due)-new Date(b.due));}
function renderReviews(){
  const list=reviewDue(); $('#review-count').textContent=`${list.length} due`; const out=$('#review-queue');
  if(!list.length){out.innerHTML='<div class="review-empty"><strong>Nothing due right now.</strong><span>Alfred will surface concepts when their next retrieval date arrives.</span></div>';return;}
  out.innerHTML=list.slice(0,8).map(r=>{const e=E.find(x=>String(x.id)===String(r.eventId)), prompt=e?.outcomes?.[0]||r.title;return `<article class="review-item" data-review-id="${esc(r.eventId)}"><span>Week ${String(r.week||'').padStart(2,'0')}</span><h3>${esc(r.title)}</h3><p><strong>From memory:</strong> ${esc(prompt)}</p><div><button type="button" data-review-rate="green">Got it</button><button type="button" data-review-rate="yellow">Shaky</button><button type="button" data-review-rate="red">Needs work</button></div></article>`}).join('');
  $$('[data-review-rate]').forEach(b=>b.onclick=()=>{const card=b.closest('[data-review-id]'), key=card.dataset.reviewId, old=state.reviews[key], interval=ratingInterval(b.dataset.reviewRate,Number(old.interval||0));old.interval=interval;old.lastRating=b.dataset.reviewRate;old.due=addDays(startOfDay(now()),interval).toISOString();old.updatedAt=new Date().toISOString();state.sessionConfidence[key]=b.dataset.reviewRate;save();saveStudyMetaToProgress(key,b.dataset.reviewRate,old);renderReviews();});
}
...
function openStuck(){
  ...
  const modal=$('#stuck-modal');modal.classList.add('open');modal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden'; $('#park-current-topic').onclick=()=>{park(topic);closeStuck();};requestAnimationFrame(()=>$('.modal-close',modal)?.focus());
}
function closeStuck(){const m=$('#stuck-modal');m.classList.remove('open');m.setAttribute('aria-hidden','true');document.body.style.overflow='';}
...
async function pushProgressRecord(key,value,updatedAt){
  try{const cfg=JSON.parse(localStorage.getItem(SYNC_KEY)||'null')||{};if(!cfg.connected||!navigator.onLine||!cfg.apiUrl||!cfg.studentKey)return;let api=String(cfg.apiUrl).trim().replace(/\/+$/,'').replace(/\/(?:health|sync)$/i,'');const sk=String(cfg.studentKey).trim().toUpperCase();if(!/^https:\/\//i.test(api)||!/^AU-(?:[A-F0-9]{4}-){9}[A-F0-9]{4}$/.test(sk))return;const res=await fetch(api+'/sync',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','Authorization':'Bearer '+sk},body:JSON.stringify({protocol:2,deviceId:cfg.deviceId||'study-page',deviceName:cfg.deviceName||'Study Page',records:[{key,value,updatedAt,deviceId:cfg.deviceId||'study-page'}]})});if(!res.ok)return;const data=await res.json();const returned=(data.records||[]).find(r=>r.key===key);if(returned&&Number(returned.updatedAt)>=updatedAt){const p=progress(),id=key.slice(6);p.events[id]=returned.value;p.recordTimes[key]=returned.updatedAt;p.updatedAt=new Date().toISOString();localStorage.setItem(PROGRESS_KEY,JSON.stringify(p));}}catch{}
}
async function syncProgressFromCloud(){
  try{const cfg=JSON.parse(localStorage.getItem(SYNC_KEY)||'null')||{};if(!cfg.connected||!navigator.onLine||!cfg.apiUrl||!cfg.studentKey)return;let api=String(cfg.apiUrl).trim().replace(/\/+$/,'').replace(/\/(?:health|sync)$/i,'');const sk=String(cfg.studentKey).trim().toUpperCase();if(!/^https:\/\//i.test(api)||!/^AU-(?:[A-F0-9]{4}-){9}[A-F0-9]{4}$/.test(sk))return;const p=progress(),records=[],did=cfg.deviceId||'study-page';const add=(key,value)=>{const updatedAt=Number(p.recordTimes?.[key]||0);if(updatedAt>0)records.push({key,value,updatedAt,deviceId:did});};Object.entries(p.events||{}).forEach(([id,v])=>add(`event:${id}`,v));Object.entries(p.weeks||{}).forEach(([w,v])=>add(`week:${w}`,v));Object.entries(p.readiness||{}).forEach(([k,v])=>add(`readiness:${k}`,v));const res=await fetch(api+'/sync',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json','Authorization':'Bearer '+sk},body:JSON.stringify({protocol:2,deviceId:did,deviceName:cfg.deviceName||'Study Page',records})});if(!res.ok)return;const data=await res.json();p.events=p.events||{};p.weeks=p.weeks||{};p.readiness=p.readiness||{};p.recordTimes=p.recordTimes||{};(data.records||[]).forEach(r=>{const lt=Number(p.recordTimes[r.key]||0),ct=Number(r.updatedAt||0);if(ct<lt)return;if(r.key.startsWith('event:'))p.events[r.key.slice(6)]=r.value||{};else if(r.key.startsWith('week:'))p.weeks[r.key.slice(5)]=r.value;else if(r.key.startsWith('readiness:'))p.readiness[r.key.slice(10)]=!!r.value;else return;p.recordTimes[r.key]=ct;});p.updatedAt=new Date().toISOString();localStorage.setItem(PROGRESS_KEY,JSON.stringify(p));hydrateReviewsFromProgress();renderRecommendation();renderWeeklyPlan();renderReviews();}
  catch{}
}
...
hydrateReviewsFromProgress();renderRecommendation();renderWeeklyPlan();renderReviews();renderParking();applyQuiet();syncProgressFromCloud();
