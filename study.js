...
function renderWeeklyPlan(){
  ...
  $('#open-week-module').href=`learn.html?week=${w}`;
...
function buildSteps(e,mode){
  ...
  const resourceHtml=resources.length?resources.map(r=>`<a class="session-resource" href="${esc(r.url)}" target="_blank" rel="noopener"><strong>${esc(r.title)}</strong><span>${esc(r.source||r.kind||'Verified resource')} · ${esc(r.priority||'Support')}</span></a>`).join(''):'<a class="session-resource" href="learn.html?week='+e.week+'"><strong>Open the Classroom</strong><span>Use the complete required teaching sequence.</span></a>';
  const steps=[];
  ...
  if(mode==='quick'){
    steps.push({title:`Core Move · ${plan[1].minutes} min`,body:`<p class="step-directive">Continue the next incomplete Classroom stage below.</p>${assignment}<div class="session-resource-list"><a class="session-resource" href="learn.html?week=${e.week}"><strong>Open Week ${String(e.week).padStart(2,'0')} Classroom</strong><span>Original lesson, required checks, media, practice, application, and mastery</span></a>${resources[0]?`<a class="session-resource" href="${esc(resources[0].url)}" target="_blank" rel="noopener"><strong>${esc(resources[0].title)}</strong><span>${esc(resources[0].priority||'Core resource')} · supporting source</span></a>`:''}</div><p class="quick-rule"><strong>Quick-session rule:</strong> use this as a 20-minute work block. The Classroom saves your exact stage. Stop at a safe point and resume later; the full lesson does not need to fit one block.</p>`});
  }else{
...
function renderSession(){
  ...
  $('#session-meta').textContent=`${steps.reduce((a,x)=>a+Number((x.title.match(/· (\d+)/)||[])[1]||0),0)} minute estimate · Pause or take a break when needed · Required: ${(target.outcomes||[]).length||'use the Classroom outcomes'} outcome(s)`;
...
function openStuck(){
  ...
  $('#stuck-options').innerHTML=`<a href="learn.html?week=${target?.week||currentWeek()}"><strong>1. Return to the exact Classroom lesson</strong><span>Use Alfred’s original explanation, worked example, and targeted correction first.</span></a><a href="${domain?`knowledge.html?domain=${domain.id}`:'knowledge.html'}"><strong>2. Simpler concept reference</strong><span>Open the Knowledge Base at the closest CETa domain.</span></a><a href="search.html?q=${encodeURIComponent(topic)}"><strong>3. Another explanation</strong><span>Search the verified video, university, manufacturer, and reference layers.</span></a><a href="standards.html"><strong>4. Find the exact weak standard</strong><span>Use evidence to repair the smallest gap instead of restarting the week.</span></a><button type="button" id="park-current-topic"><strong>5. Park the side question</strong><span>Save this topic and go back to the required task.</span></button>`;
