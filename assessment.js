...
function availableCount(d,track){const codes=d[track==='CETa'?'ceta':'career']||[];return Math.min(d.mix?.[track]||0,D.questions.filter(q=>q.masteryEvidence!==false&&q.difficulty!=='Orientation'&&q.track===track&&(!codes.length||q.standards.some(s=>codes.includes(s.replace(/^CETA:|^CAREER:/,''))))).length)}
function card(kind,d){const a=recordFor(kind,d.id,d.week), attempts=a?.attempts||[], taken=Number(a?.attemptCount||attempts.length||0), url=`quiz.html?type=${kind}&id=${encodeURIComponent(d.id)}`,c=availableCount(d,'CETa'),r=availableCount(d,'Career'),short=c+r<d.count;return `<article class="assessment-launch-card"><div class="assessment-launch-head"><span>${kind==='lesson'?'Lesson Quiz':kind==='lab'?'Lab Check':kind==='week'?'Weekly Test':'Major Assessment'}</span>${statChip(a,d.target||80)}</div><h3>${esc(d.title)}</h3><p>${c+r} questions · ${c} CETa / ${r} career${short?' · Short practice form':''}</p>${short?'<p>Some questions were retired. This shorter form is practice, not a full-length readiness test.</p>':''}<div class="assessment-tags"><span>${(d.ceta||[]).length} CETa standards in scope</span><span>${(d.career||[]).length} career standards in scope</span><span>${d.target||80}% practice target</span></div><a class="button ${taken?'outline-green':'green'}" href="${url}">${taken?'Retake / New Form':'Start Assessment'} →</a></article>`}
function init(){
  $('#assessment-kpis').innerHTML=`<div><strong>${D.lessonQuizzes.length}</strong><span>lesson quizzes</span></div><div><strong>${D.labQuizzes.length}</strong><span>lab checks</span></div><div><strong>${D.meta.audit?.answerChecked||0}</strong><span>numerical answers checked</span></div><div><strong>${D.meta.gradedQuestionCount.toLocaleString()}</strong><span>active practice questions · audit incomplete</span></div>`;
