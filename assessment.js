...
function qualifyingRuns(record,d){return (record?.attempts||[]).filter(at=>String(at?.b||'')===String(D.meta?.version||'16.1')&&Number(at?.total||0)>=Number(d?.count||100)&&Number(at?.pct||0)>=Number(d?.qualifyingTarget||85)).length}
function card(kind,d){
 ...
 const readiness=d.id==='ceta-mock'?`<div class="readiness-run-status"><strong>${Math.min(qualifyingRuns(a,d),d.requiredRuns||2)} of ${d.requiredRuns||2} qualifying current runs</strong><span>Each must be a separate full ${d.count}-question run at ${d.qualifyingTarget||85}% or higher.</span></div><p>${esc(d.readinessRule||'')}</p>`:'';
