...
function init(){
  $('#assessment-kpis').innerHTML=`<div><strong>${D.lessonQuizzes.length}</strong><span>lesson quizzes</span></div><div><strong>${D.labQuizzes.length}</strong><span>lab checks</span></div><div><strong>${D.weeklyTests.length}</strong><span>weekly mastery tests</span></div><div><strong>${D.meta.questionCount.toLocaleString()}</strong><span>original questions</span></div>`;
