...
function card(kind,d){if(d.noQuiz)return `<article class="assessment-launch-card"><h3>${esc(d.title)}</h3><p>${esc(d.guidance)}</p><a class="button outline-green" href="week.html?week=${d.week}">Open scheduled work →</a></article>`;const a=recordFor(kind,d.id,d.week), attempts=a?.attempts||[], taken=Number(a?.attemptCount||attempts.length||0), url=`quiz.html?type=${kind}&id=${encodeURIComponent(d.id)}`,c=availableCount(d,'CETa'),r=availableCount(d,'Career'),short=c+r<d.count;return `<article class="assessment-launch-card"><div class="assessment-launch-head"><span>${kind==='lesson'?'Optional Week Practice':kind==='lab'?'Lab Check':kind==='week'?'Optional Weekly Review':'Major Assessment'}</span>${statChip(a,d.target||80)}</div><h3>${esc(d.title)}</h3><p>${c+r} questions · ${c} CETa / ${r} career${short?' · Short practice form':''}</p>${short?'<p>Some questions were retired. This shorter form is practice, not a full-length readiness test.</p>':''}<div class="assessment-tags"><span>${(d.ceta||[]).length} CETa objectives sampled</span><span>${(d.career||[]).length} career objectives sampled</span><span>${d.target||80}% practice target</span></div><a class="button ${taken?'outline-green':'green'}" href="${url}">${taken?'Retake / New Form':'Start Assessment'} →</a></article>`}
function init(){
  $('#assessment-kpis').innerHTML=`<div><strong>${D.lessonQuizzes.length}</strong><span>lesson quizzes</span></div><div><strong>${D.labQuizzes.length}</strong><span>lab checks</span></div><div><strong>${D.meta.audit?.retainedNumericalItems||0}</strong><span>numerical answers checked</span></div><div><strong>${D.meta.gradedQuestionCount.toLocaleString()}</strong><span>editorially reviewed practice items</span></div>`;
